---
title: "Lab 99: Intro to shiny documents"
...
Once we have the required subset, we can use `ggplot()` to graph a timeline trend: 
```{r}
ggplot(data = mynames, aes(x = year, y = n)) +
  geom_line(size = 1)
```

Because some names appear in both males and females, we can add color to the 
line(s) based on sex:
```{r}
ggplot(data = mynames, aes(x = year, y = n)) +
